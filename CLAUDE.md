...
**poetryのバージョンによって、新規作成する`.venv`に`pip`自体をseedするかどうかのデフォルトが違う** — poetry 2.1.2→2.4.1への更新を試みた際、`.venv`に`pip`(そのバージョンが内部でvendoringしている古い`msgpack`ごと)が紛れ込み、Docker Scoutで`msgpack`/`setuptools`のfixed-version脆弱性が新たに検出される回帰が発生した(`.venv/bin/pip show`では両パッケージとも"not found"になるため、実体は`pip`自身が`pip/_vendor/msgpack`として内包している埋め込みコードであり、独立にインストールされたパッケージではないことを`find`で全ファイルシステムを検索して確認した)。`POETRY_VIRTUALENVS_OPTIONS_NO_PIP`のような環境変数指定はpoetry側の既知の制約で`virtualenvs.options.*`のようなネストした設定には効かないことがあるため、`dependencies`ステージで明示的に`poetry config virtualenvs.options.no-pip true` / `no-setuptools true`を実行し、設定ファイル経由で確実に反映させている。このアプリはランタイムに`pip`/`setuptools`を一切必要としないため、`.venv`から除外して問題ない。
